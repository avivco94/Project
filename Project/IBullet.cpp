...
IBullet::IBullet(std::string id, std::string p, sf::IntRect rect, sf::Vector2f startPos, sf::Vector2f direction, float rotation, float durtion, float speed, std::string type)
	: IHitWeapons(id, p), m_startPos(startPos), MoveableSpriteObject(*Resources::getInstance().getTexturesMap()->getResource(BULLETS_TEXTURE), rect, startPos, speed), m_direction(direction), m_rotation(rotation), m_duration(durtion), m_passed(0, 0), m_type(type) {
...
std::shared_ptr<SerializableInfo> IBullet::getInfo()
{
...
std::type_index IBullet::type() const {
	return typeid(IBullet);
}
